...
- **页级重译 `--retranslate`**：`--retranslate 3` 只重付第 3 页的 LLM 调用（其余页全部缓存命中），新译文覆盖进缓存、渲染结果缓存自动失效重渲染——修单页译文的成本从"整册重跑"降到"单页增量"；UI/嵌入调用亦可经事件总线命令 `post("retranslate", pages=[...])` 运行中到达（流式路径页粒度生效）
- **CLI 友好报错**（FAILPATHS I8 收口）：坏 PDF/文件不存在/配置语法错误等可预期失败打印一行 ERROR + 指向性提示（退出码 2），不再甩裸 traceback；未预期异常仍保留完整堆栈便于报 bug
- **项目级缓存库**：翻译缓存 + 版面缓存 + 位图裁剪缓存 + **整页渲染结果缓存** + 文档指纹索引五合一（`.pdf_translator_cache/cache.db`，默认随输入文件目录）——同一输入译到不同输出目录共享缓存，文档按内容指纹索引（复制/改名不失效）；配合翻译缓存形成"改 1 页只重译 1 页"的文档增量翻译能力；公式/图/表 300dpi 位图按（指纹,页,区域,dpi）内容寻址缓存——重跑 0 调用也不再重裁图（512MB 上限 LRU 淘汰）；**最终输出 PDF 按渲染输入全量哈希内容寻址缓存**（`performance.render_cache` 默认开）——翻译/布局全命中的热跑直接回放最终产物，渲染+落盘 0 计算（paper3 8 页干跑实测 10.9s → 0.45s），改任何渲染相关配置或译文变化自动失效重渲染；旧 `.translation_cache.db` 首次运行自动迁移；`performance.cache_dir` 可指定位置
- **页级整页排版**（`render.page_story: auto`）：faithful 模式下整页段落**与表格宽格**进单个 Story 流式排版引擎逐项落位（页几何冻结，框位置不变）——段间基线共享连续网格，消除逐段排版的 baseline 抖动；紧段（收缩因子 ≥0.78）以确定性局部字号收编、深缩格（≥0.3）并入 Story，一次字体重解析替代逐段/逐格 insert_htmlbox（实测 8 页论文全缓存重渲染 45s → 10s）。页级预检+落墨前预演双防线：深溢出段（因子 <0.78）整页回退逐段排版（引擎深缩+告警，绝不段级混排、绝不串位），回退计数进日志
- **流畅阅读模式**（`output.mode: reflow`）：整文档语义重排——跨页组装阅读序（段落/公式位图/图区+图注绑定/表格/Algorithm 框/语义化列表 `<ol>`/`<ul>`），页面模板沿用原文档页尺寸/边距/栏结构（可配单栏），Story 整文档流式写入自动断页；预设样式表（正文随原字号/1.4 行距、H1 16、H2 13、图注 8.5、文献 9pt **带 14pt 悬挂缩进**），无 fit 因子无降级阶梯——排版质量由内容流自然产生；高置信表 HTML 语义重排（译文入表）、低置信表整表位图；被图区吞掉的作者块/机构行纵向并块后以单一段落回流（不碎片化）；已烘焙进位图/表格的文字绝不重复回归文流；自动断页+页码+PDF 书签（`doc.set_toc`）；超长文档按章节边界分段 Story 防内存；输出名带 `-reflow` 后缀；翻译层与 faithful 完全共用（同缓存同调用数）
...
- **正文链接存活**：redaction 会摧毁正文区链接注释（引用跳转/DOI/URL），渲染层先存后补原位重插——译文输出保留原文档全部可点击链接；**流畅阅读（reflow）模式同样恢复链接**：源页链接按文本归属映射进重排文流（译文内精确定位引文标记/URL/DOI 并以 `<a>` 包裹，锚点解析后重插注释），引文 GoTo 跳转落到重排后的文献条目/公式/图注新坐标——实测 8 页双栏论文 64 链接恢复 61（余 3 个在位图区/列表项内无文本块可挂，计数进警告不阻塞出片）
- **请求墙钟·楔死终结**：真实网关偶发把请求楔死在响应头等待阶段（httpx read timeout 在该路径不触发，单批可挂 25 分钟+无异常）。LLM 连接池自持可终结句柄（CLI/UI/连通性测试三入口统一 `LLMClientPool`），请求在守护线程执行并设独立墙钟（2×超时+30s）——到点终结旧连接池强制解堵、换新池按既有退避重试；SDK 层 `max_retries=0` 重试单层化（SDK 盲重试 × 自有重试的时延乘法废止，实测 5s 超时曾被放大到 29.7s）；依赖按所装 SDK 自动选 httpx2/httpx 栈
- **输出字体子集化**（`performance.subset_fonts` 默认开）：按文档用字子集化嵌入字体（fontTools 可选依赖，未装自动跳过并提示）——实测 8 页译文输出 15MB → 0.9MB（-94%），文本逐字节一致、链接/书签/渲染像素零损失
...
`--retranslate 3` / `--retranslate 1,3-4`：指定页的
全部翻译单元绕过翻译缓存强制重译——修完某页译文后只重付该页调用，其余
页全部缓存命中；新译文自动覆盖进缓存，整页渲染结果缓存按译文内容寻址
自动失效重渲染。
...
289 个单测覆盖核心逻辑：流式增量解析与断流、批内分段重试、句子级缓存、术语确定性修复、多引擎投票、几何版面分割、影子页 GNN、流水线重叠端到端一致性（含 跨页合并修复回归）、嵌套表地狱样张（合并表头/右对齐数字列/跨页延续）与置信度分级、批字符预算组批、缓存容量淘汰与命中统计、单元格原文回灌与 htmlbox 单元格回灌、扫描页 OCR（附录/原位）、布局并行与串行一致性、任务队列/history 归档/重启恢复/历史字段迁移、SSE 端点帧契约与 Last-Event-ID 重放、配额自适应换算、htmlbox/ writer 双渲染路径与 RTL 样张、排版自适配（测量基座/样式级因子/降级阶梯/扩框零重叠/微升/丢段兜底）、源头控长（预算规则/单段重问/预算档缓存）、small caps 节标题与整块粗体摘要的样式回归、OCR 行分组/图形避让/原位回贴、pymupdf-layout 适配层（1.28.x 五元组 + 未装回退）、provider 参数透传、页级 Story 整页落位/溢出回退/开关组合/局部收缩收编/单元格并入、流式停顿墙钟与看门狗（滴漏/静默两型）、双语表格语义布局（两行同框/紧框回退/标题豁免）、redaction 链接存活（重插持久化/不重复）、请求墙钟楔死终结（流式/非流式头部/裸 client/池代数防护）、reflow 链接重映射（URI 直通/引文 GoTo 锚点解析/译文内定位/退化不阻塞）、reflow 模板统计与文档模型（栏数/书签流序/列表语义化/图注绑定/文献悬挂缩进）、reflow 端到端流式写入（多页断页/页码/书签/位图守恒）、跨页合并整段、超页高表缩放、RTL 方向、项目级缓存库（跨输出目录共享/legacy 迁移/指纹索引/热跑不重写布局/io.pages 子集版面条目隔离）、io.pages 子集、命令总线（stage 标签/订阅/检查点消费）、validate-key 成功分支、config 空节点容错、跨页断句拆分（含边界标点与连字符合并）、公式编号剥离、Algorithm 框判定、三线表检测、页脚阈值、渲染回灌、多语言注册表/跨平台字体解析/Unicode 断行、服务端目录浏览与输出预览、key 回填、reflow 分段配置生效/畸形链接防御/流式失败批终结、reflow×Typography 缺失兜底/零内容空白页守卫/模板退化栏守卫/OCR 临时文件句柄收口/队列起跑失败返回/空段节点容错/OCR 引擎名白名单、单栏文档栏型误判（整宽段腰斩根因修复）/单栏整宽三线表与算法框检出/窄小单元格不 redact 原像素保留/UI 缓存节省报表字段全链路（Job→snapshot→DB 迁移）/PUT config 空段节点/UI worker 干跑告警与超时兜底/运行中页级重译命令·io.pages 子集重译映射·fonts.body 接线·无字体环境 reflow 位图兜底·加粗标题伪代码豁免·伪代码框公式条吸收等。
测试不需要网络和 API key。
...
- **页级整页排版**（`render.page_story`，默认 auto）按页预检：深溢出段（收缩因子 <0.78）该页自动整页回退逐段排版（引擎深缩+告警，日志有计数），正确性不受影响；紧段以确定性局部字号收编（告警可见）；双语模式与 `writer` 引擎不走页级 Story
...
- **流畅阅读模式（reflow）已知限制**：页面对应关系不存在（重排后无法按原页引用）；与双语模式互斥（配置期报错）；扫描页（OCR 任务会触发）在翻译开始前即报错——不烧任何 LLM 调用；Algorithm 环境与正文交错的页，伪代码碎片以独立位图随文流（非整框重排）；跨页表在 reflow 中按页内碎片呈现；链接大部分恢复（实测 64 恢复 61）——烘焙进位图区/列表项内的少量链接无文本块可挂（计数进警告）；单元格内链接暂不支持

...
- **正文链接存活**：redaction 会摧毁正文区链接注释（引用跳转/DOI/URL），渲染层先存后补原位重插——译文输出保留原文档全部可点击链接；**流畅阅读（reflow）模式同样恢复链接（v0.8.3）**：源页链接按文本归属映射进重排文流（译文内精确定位引文标记/URL/DOI 并以 `<a>` 包裹，锚点解析后重插注释），引文 GoTo 跳转落到重排后的文献条目/公式/图注新坐标——实测 8 页双栏论文 64 链接恢复 61（余 3 个在位图区/列表项内无文本块可挂，计数进警告不阻塞出片）
- **请求墙钟·楔死终结**（v0.8.3）：真实网关偶发把请求楔死在响应头等待阶段（httpx read timeout 在该路径不触发，单批可挂 25 分钟+无异常）。LLM 连接池自持可终结句柄（CLI/UI/连通性测试三入口统一 `LLMClientPool`），请求在守护线程执行并设独立墙钟（2×超时+30s）——到点终结旧连接池强制解堵、换新池按既有退避重试；SDK 层 `max_retries=0` 重试单层化（SDK 盲重试 × 自有重试的时延乘法废止，实测 5s 超时曾被放大到 29.7s）；依赖按所装 SDK 自动选 httpx2/httpx 栈
- **输出字体子集化**（v0.8.3，`performance.subset_fonts` 默认开）：按文档用字子集化嵌入字体（fontTools 可选依赖，未装自动跳过并提示）——实测 8 页译文输出 15MB → 0.9MB（-94%），文本逐字节一致、链接/书签/渲染像素零损失
...
- **流畅阅读模式（reflow）已知限制**：页面对应关系不存在（重排后无法按原页引用）；与双语模式互斥（配置期报错）；扫描页（OCR 任务会触发）在翻译开始前即报错——不烧任何 LLM 调用；Algorithm 环境与正文交错的页，伪代码碎片以独立位图随文流（非整框重排）；跨页表在 reflow 中按页内碎片呈现；链接大部分恢复（v0.8.3：实测 64 恢复 61）——烘焙进位图区/列表项内的少量链接无文本块可挂（计数进警告）；单元格内链接暂不支持

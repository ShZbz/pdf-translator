...
- **流畅阅读模式**（`output.mode: reflow`）：整文档语义重排——跨页组装阅读序（段落/公式位图/图区+图注绑定/表格/Algorithm 框/语义化列表 `<ol>`/`<ul>`），页面模板沿用原文档页尺寸/边距/栏结构（可配单栏），Story 整文档流式写入自动断页；预设样式表（正文随原字号/1.4 行距、H1 16、H2 13、图注 8.5、文献 9pt），无 fit 因子无降级阶梯——排版质量由内容流自然产生；自动断页+页码+PDF 书签（`doc.set_toc`）；超长文档按章节边界分段 Story 防内存；输出名带 `-reflow` 后缀；翻译层与 faithful 完全共用（同缓存同调用数）
...
- **流畅阅读模式（reflow）已知限制**：页面对应关系不存在（重排后无法按原页引用）；与双语模式/扫描页（OCR）互斥（配置报错）；Algorithm 环境与正文交错的页，伪代码碎片以独立位图随文流（非整框重排）；跨页表在 reflow 中按页内碎片呈现；文献条目无悬挂缩进（预设样式表 v1）
